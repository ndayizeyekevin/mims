...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        try {
            ...
            $stmt = $connection->prepare("SELECT password FROM users WHERE insurance_user_id = :user_id");
            ...
            if ($user && password_verify($current_password, $user['password'])) {
                ...
                $stmt_update = $connection->prepare("UPDATE users SET password = :new_password WHERE insurance_user_id = :user_id");
